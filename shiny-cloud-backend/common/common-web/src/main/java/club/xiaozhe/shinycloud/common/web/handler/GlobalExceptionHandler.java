...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<Result<Void>> handleJsonParseError(HttpMessageNotReadableException ex) {
        ...
        if (upe != null) {
            ...
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(Result.error(400, "未知字段 \"" + unknownField + "\"，支持的字段：" + knownFields));
        }

        log.warn("JSON 解析失败，异常链:", ex);
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(Result.error(400, "请求体格式错误，请检查 JSON 格式"));
    }
    ...
    public ResponseEntity<Result<Void>> handleMethodArgumentNotValidException(MethodArgumentNotValidException e) {
        ...
        return ResponseEntity.status(code.getStatus())
                .body(Result.error(code.getCode(), firstErrorMessage));
    }
    ...
    public ResponseEntity<Result<Void>> handleBusinessException(BusinessException e) {
        ...
        return ResponseEntity.status(code.getStatus())
                .body(Result.error(code.getCode(), e.getMessage()));
    }
    ...
    public ResponseEntity<Result<Void>> handleRuntimeException(RuntimeException re) {
        log.error("发生未知错误", re);
        ErrorCode code = ErrorCode.UNKNOWN_ERROR;
        return ResponseEntity.status(code.getStatus())
                .body(Result.error(code.getCode(), code.getMessage()));
    }
}
